...
import { financialChartData } from '../../../../../public/data';
...
@Component({
  ...
  imports: [HeaderComponent, ChartAllModule, TabAllModule, TabModule],
...
export class FinancialComponent {
  ...
  public rangeLoad(args: ILoadedEventArgs): void {
    ...
    this.rangeIntervalId = setInterval(() => {
      ...
      if (container && container.children.length > 0 && container.id === args.chart.element.id) {
        const newData = this.getRangeColumnData().map((item: any) => ({
          x: item.x,
